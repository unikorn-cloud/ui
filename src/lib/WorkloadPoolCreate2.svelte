...
{#if autoscaling}
	...
	<p>
		Define the minimum pool size. When zero, the pool will not consume any resources when not in
	...
	<div class="flex gap-8">
		<RangeSlider class="grow" min="0" max="100" step="1" bind:value={sizeMin} />
		<span>{sizeMin}</span>
	</div>

	<h6 class="h6">Maximum Pool Size</h6>
	<p>Define the maximum pool size.</p>
	<div class="flex gap-8">
		<RangeSlider class="grow" min="1" max="100" step="1" bind:value={sizeMax} />
		<span>{sizeMax}</span>
	</div>
{:else}
	<h6 class="h6">Pool Size</h6>
	<p>Define the pool size</p>
	<div class="flex gap-8">
		<RangeSlider class="grow" min="1" max="100" step="1" bind:value={sizeMax} />
		<span>{sizeMax}</span>
	</div>
