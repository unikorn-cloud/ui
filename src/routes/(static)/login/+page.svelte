<script lang="ts">
	...
	if (browser) {
		...
		if (location.searchParams.has('state')) {
			state = location.searchParams.get('state') || 'undefined';
		}
	}
		...
		<form id="login_form" class="flex flex-col gap-8" method="post" action={callback}>
			...
			<section class="flex flex-col gap-4">
				...
				<input
					...
					autocomplete="email"
				/>
				<input class="btn variant-ghost-primary" type="submit" value="Login" />
			</section>

			<section class="flex flex-col gap-4">
				<p>or login with your chosen provider</p>
				<button
					class="btn variant-ghost-primary flex align-center"
					on:click={google}
					on:keypress={google}
				>
